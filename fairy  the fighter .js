...
let x = 500,
  y = 700; // Initial position of the fairy

let littleMonsters = [];
let lastSpawnTime = 0; // Tracks the last spawn time for little monsters
let spawnInterval = 3000; // Spawn every 3 seconds
...
function gameScreen() {
  ...
  for (let i = littleMonsters.length - 1; i >= 0; i--) {
    ...
    if (dist(monster.x, monster.y, x, y + 90) < 30) {
...
function moveLittleMonster(monster) {
  let speed = 2; // Speed of the little monsters
  let angle = atan2(y + 90 - monster.y, x - monster.x); // Direction towards the fairy
  monster.x += cos(angle) * speed;
  monster.y += sin(angle) * speed;
}
// *Draw little monster*
function drawLittleMonster(monsterX, monsterY) {
  fill(255, 100, 0); // Orange color for little monsters
  ellipse(monsterX, monsterY, 40, 40); // Body
  fill(0);
  ellipse(monsterX - 10, monsterY - 10, 10, 10); // Eye
  ellipse(monsterX + 10, monsterY - 10, 10, 10); // Eye
  arc(monsterX, monsterY + 10, 20, 10, 0, PI); // Mouth
}
